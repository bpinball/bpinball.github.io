JSSpeccy.Keyboard = function() {
	...
	self.poll = function(addr) {
		let pad = navigator && navigator.getGamepads()[0];
		var result = 0xff;
		for (var row = 0; row < 8; row++) {
			if (!(addr & (1 << (row+8)))) { /* bit held low, so scan this row */
				result &= keyStates[row];
				if (pad) {
					if (row == 5 && pad.buttons[0].pressed) {
						result &= ~(0x01); // keyCode 80 - 'P'
					}

					if (row == 0) {
						if (pad.axes[0] == -1) {
							result &= ~(0x02); // keyCode 90 - 'Z'
						} else if (pad.axes[0] == 1) {
							result &= ~(0x04); // keyCode 88 - 'X'
						}
					} 

					if (row == 6 && pad.axes[1] == -1) {
						result &= ~(0x02); // keyCode 76 - 'L'
					}

					if (row == 7 && pad.axes[1] == 1) {
						result &= ~(0x02); // keyCode 17 - 'sym'
					}
				}
			}
		}
